...
function sourcesToBuild(buildPath = '_build') {
	const pjsonPath = './' + buildPath + '/package.json';
	const pljsonPath = './' + buildPath + '/package-lock.json';

	[pjsonPath, pljsonPath]
		.map((path) => ({ path, content: require(path) }))
		.map(({ path, content }) => ({ path, content: renamePackage(content) }))
		.map(({ path, content }) => ({ path, content: removeScripts(content) }))
		.map(({ path, content }) => write(path)(content));
}

function removeScripts(pjson) {
